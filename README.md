...
A simple example of a Java SoapServlet and a JSP. 
...
![Example SoapServlet architecture](images/servlet-diag.gif)

## Java SoapServlet Pages (JSP)
...
## Running the SoapServlet

To run the SoapServlet build the war file, then deploy using Jetty or Tomcat.

To build the SoapServlet we build the ```package``` target using Maven.  From the command line the command is -
